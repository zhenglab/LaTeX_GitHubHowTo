...
Git有两种取消跟踪忽略文件的方式，一种是建立\verb|.gitignore|文件，另一种是修改\verb|exclude|文件。
\begin{description}
...
此时，将不会再跟踪这两类文件。设置用户的\verb|.gitignore|文件可以使你所有的项目都忽略掉你所设置的忽略文件。具体做法可参考\cite{5:misc}。为了方便实验室其他人能共享忽略文件，所以在每个项目中都包含忽略文件的方式更适合我们使用。
\item[exclude] 你也可以在你的项目中的.git/info/exclude中设置你的忽略文件，但是这个设置只是当前用户的设置，其他人无法共享，所以这种方式我们不采用。
\end{description}
...
\end{bash}
\item 从远程抓取数据：\lstinline|git fetch upstream|，此时目录中就出现了远程仓库中的文件。
\end{itemize}
...
\item 填写库名(比如Test)和描述，选择Public或者是Private，在Initialize this repository with a README选项前打勾，然后点击Create repository，如图\ref{fig:1}。
\begin{figure}[!htb] %插图
\centering
